...
template< class T >
inline T Vector2<T>::operator * (const Normal3<T>& n) const {
	return x * n.x + y * n.y; // dot product
}

template< class T >
inline T Vector2<T>::operator * (const Point2<T>& p) const {
	return x * p.x + y * p.y + z * p.z; // dot product
}
